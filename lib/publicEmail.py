...
#{'email_pass': 'Socl&*123;cdp@!', 'db_user': 'conf/user.txt', 'email_user': 'server@lmbang.com', 'smtp_server': 'smtp.exmail.qq.com', 'db_pass': 'conf/pass.txt', 'rate': '20000', 'ip_file': 'conf/ip.txt', 'target_email': 'xiaozhiqi@lmbang.com', 'type': 'all'}


class Send_Email:
    def __init__(self, conf):
        self.user, self.password, self.smtp_server, self.smtp_port, self.toemail, self.xlsfile, \
        self.change_add_list, self.change_del_list, self.weakpass_result = \
            conf['email_user'], conf['email_pass'], conf['smtp_server'], conf['smtp_port'], conf['target_email'], \
    ...
    def send(self):
        ...
        if self.change_add_list or self.change_del_list or self.weakpass_result:
            ...
            part = MIMEText("端口服务详情请参照附件信息。\n注：端口服务信息并未改变，且不存在弱口令信息",'plain','utf-8')
            msg.attach(part)
        if self.xlsfile:
        ...
        while True:
            ...
            try:
                ...
                result = s.login(self.user, self.password)
            ...
            #except smtplib.SMTPException, e:
                ...
                error += 1
                logger.infostring('sending mail failure,error: %s' % e)
                continue
...
#    a = Send_Email({'email_pass': 'Socl&*123;cdp@!', 'email_user': 'server@lmbang.com', 'smtp_server': 'smtp.exmail.qq.com','target_email': 'xiaozhiqi@lmbang.com'})
#    a.run()
